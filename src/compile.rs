...
fn compile_value(value : ASTNode, compiler : &mut Compiler) -> Value
{
    ...
    {
        return Value::Add(Box::new(compile_value(*lhs, compiler)), Box::new(compile_value(*rhs, compiler)))
    }
...
fn compile_node(node : ASTNode, compiler : &mut Compiler) -> Vec<Operand>
{
    ...
    match node
    {
        ...
        ASTNode::FunctionDeclaration(ty, name, inner, params) => {
            statements.push(Operand::FunctionDecl(ty.into_ir(), name, compile_list(inner, compiler), params.iter().cloned().map(|v| (v.0, v.1.into_ir())).collect()));
        },
        ASTNode::VariableDeclaration(ty, name, value) =>
        {
            statements.push(Operand::DeclareVariable(ty.into_ir(), name, compile_value(*value, compiler)));
        },
        ASTNode::Add(lhs, rhs) =>
        {
            statements.push(Operand::Add(OperandType::Int(Size::DoubleWord), compile_value(*lhs, compiler), compile_value(*rhs, compiler)));
        }
        ASTNode::Sub(lhs, rhs) =>
        {
            statements.push(Operand::Subtract(OperandType::Int(Size::DoubleWord), compile_value(*lhs, compiler), compile_value(*rhs, compiler)));
        }
